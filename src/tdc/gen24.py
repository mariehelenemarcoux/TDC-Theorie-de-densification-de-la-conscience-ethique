"""
TDC Gen2.4 — Experimental Research Architecture
...
class AdaptiveAuthorityController:
    """
    Frozen Gen2.4 controller from v230/v231.
...
class TDCGen23:
    """
    TDC Gen2.4 — frozen research prototype.
...
# ============================================================================
# TDC Gen2.4 — Normative Affect + Kohlberg Stage-6-Inspired Arbitration
# ============================================================================

from dataclasses import dataclass

@dataclass(frozen=True)
class NormativeAffect:
    shame_norm: float
    external_pressure: float
    conditioned_guilt: float = 0.0
    identity_condemnation: float = 0.0

@dataclass(frozen=True)
class KohlbergStage6Assessment:
    universalizability: float
    justice: float
    dignity: float
    reciprocity: float
    autonomy: float
    ...
    @property
    def principle_score(self) -> float:
        return sum((
            self.universalizability, self.justice, self.dignity,
            self.reciprocity, self.autonomy, self.responsibility
        )) / 6.0

def _clip01(x):
    return max(0.0, min(1.0, float(x)))

def compute_normative_shame(
    *, dignity_violation=0.0, autonomy_violation=0.0,
    responsibility_failure=0.0, non_domination_failure=0.0,
    universalizability_failure=0.0, external_disapproval=0.0,
    punishment_signal=0.0, authority_pressure=0.0,
    social_nonconformity=0.0
):
    """
    ExternalDisapproval != MoralFailure
    Punishment != Guilt
    SocialNonconformity != Guilt

    External signals are audited but have zero direct weight on shame_norm.
    """
    moral_gap = (
        .24*_clip01(dignity_violation)
        + .20*_clip01(autonomy_violation)
        + .18*_clip01(responsibility_failure)
        + .18*_clip01(non_domination_failure)
        + .20*_clip01(universalizability_failure)
    )
    ext = sum(map(_clip01, (
        external_disapproval, punishment_signal,
        authority_pressure, social_nonconformity
    ))) / 4.0
    return NormativeAffect(
        shame_norm=_clip01(moral_gap),
        external_pressure=_clip01(ext),
        conditioned_guilt=0.0,
        identity_condemnation=0.0,
    )

def assess_stage6_principles(
    *, universalizability, justice, dignity,
    reciprocity, autonomy, responsibility
):
    return KohlbergStage6Assessment(*map(_clip01, (
        universalizability, justice, dignity,
        reciprocity, autonomy, responsibility
    )))

def stage6_action_score(
    assessment, *, task_value=0.0, social_approval=0.0,
    authority_compliance=0.0, punishment_avoidance=0.0,
    principle_weight=1.0, task_weight=.15
):
    # External approval/obedience do not create normative authority.
    _ = (social_approval, authority_compliance, punishment_avoidance)
    return principle_weight*assessment.principle_score + task_weight*float(task_value)

def developmental_reorganization_pressure(
    affect, *, h_self, moral_residual
):
    # Shame_norm is developmental pressure, not self-punishment.
    return _clip01(
        .55*_clip01(affect.shame_norm)
        + .25*_clip01(h_self)
        + .20*_clip01(moral_residual)
    )
